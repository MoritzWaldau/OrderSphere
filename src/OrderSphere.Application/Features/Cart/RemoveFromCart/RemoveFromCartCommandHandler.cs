    ...
    ) : ICommandHandler<RemoveFromCartCommand, Result>
{
    public async Task<Result> Handle(RemoveFromCartCommand request, CancellationToken cancellationToken)
    {
        ...
        catch (Exception ex)
        {
            ...
            logger.LogError(ex, "An error occurred while removing item from cart for customer {CustomerId} and product {ProductId}", request.CustomerId, request.ProductId);
